...
CREATE TABLE IF NOT EXISTS `cloudchatdb`.`User_settings` (
  `show_previews` TINYINT(1) NULL DEFAULT 1,
  `highlight_words` VARCHAR(1024) NULL,
  ...
  `autohide_channels` TINYINT(1) NULL DEFAULT 0,
  `hide_joinpartquit_messages` TINYINT(1) NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `cloudchatdb`.`IRC_servers` (
  ...
  `serverPort` VARCHAR(45) NOT NULL DEFAULT '6667',
...
CREATE TABLE IF NOT EXISTS `cloudchatdb`.`IRC_channel_messages` (
  ...
  `timeReceived` DATETIME NOT NULL,
  `seen` DATETIME NULL,
  `IRC_channels_channelID1` INT NOT NULL,
  PRIMARY KEY (`messageID`, `IRC_channels_channelID1`),
  INDEX `fk_IRC_channel_messages_IRC_channels2_idx` (`IRC_channels_channelID1` ASC),
  CONSTRAINT `fk_IRC_channel_messages_IRC_channels2`
    FOREIGN KEY (`IRC_channels_channelID1`)
